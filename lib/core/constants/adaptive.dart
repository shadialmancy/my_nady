...
class AdaptiveUi {
  ...
  /// 0 - 1000
  static const WidthPlatformBreakpoint small = WidthPlatformBreakpoint(
    begin: 0,
    end: mediumBreakpoint,
  );

  /// 0+
  static const WidthPlatformBreakpoint smallAndUp = WidthPlatformBreakpoint(
    begin: 0,
  );
  ...
  /// 1000+
  static const WidthPlatformBreakpoint mediumAndUp = WidthPlatformBreakpoint(
    begin: mediumBreakpoint,
  );

  /// 1000 - 1200
  static const WidthPlatformBreakpoint medium = WidthPlatformBreakpoint(
    begin: mediumBreakpoint,
    end: largeBreakpoint,
  );

  /// 1200+
  static const WidthPlatformBreakpoint largeAndUp = WidthPlatformBreakpoint(
    begin: largeBreakpoint,
  );
...
class CustomAdaptiveScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ScreenTypeLayout.builder(
      watch: (watchWidget != null)
          ? (BuildContext context) => watchWidget
          : null,
...
class WidthPlatformBreakpoint {
  final double begin;
  final double? end;
  final Set<TargetPlatform>? platform;

  const WidthPlatformBreakpoint({required this.begin, this.end, this.platform});

  bool matches(double width, TargetPlatform targetPlatform) {
    final inRange = width >= begin && (end == null || width < end!);
    final platformMatch =
        platform == null || platform!.contains(targetPlatform);
